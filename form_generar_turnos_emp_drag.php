...
<body>
    <div class="container">
        <h1 class="mt-5">Generar Turnos para Empleados</h1>
        <form action="form_generar_turnos_emp_drag.php" method="post" class="mt-4">
        ...
        <?php
            if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['filtrar'])) {
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo '<tr>';
                        echo '<td><input type="text" name="empleado_id[]" value="'.$row['empleado_id'].'" class="form-control" /></td>';
                        echo '<td>' . $row['nombre'] . ' ' . $row['apellido'] . '</td>';
                        echo '<td><input type="text" name="horas_contratadas[]" value="'.$horas_contratadas.'" class="form-control" /></td>';
                        echo '<td><input type="text" name="horas_a_laborar_mes[]" value="'.$horas_laborar.'" class="form-control horas-a-laborar" /></td>';
                        
                        for ($i = 1; $i <= $dias_mes; $i++) {
                            echo '<td class="day-column">
                                <button type="button" class="btn btn-info btn-sm open-modal-btn">Seleccionar Turno</button>
                                <input type="text" class="form-control turno-input" name="turno_dia[' . $row['empleado_id'] . '][' . $i . ']" value="" readonly>
                            </td>';
                        }
                        
                        echo '<td class="day-column">
                            <button type="button" class="btn btn-info btn-sm open-modal-btn">Seleccionar Turno</button>
                            <input type="text" class="form-control turno-input" name="turno_dia[' . $row['empleado_id'] . '][next_month_1]" value="" readonly>
                        </td>';
            ...
            <div id="turnosListModal">
                <?php
                    ...
                    while ($turno = $turnos_result->fetch_assoc()) {
                        echo '<div class="turno draggable" data-cod-turno="' . $turno['cod_turno'] . '">' . $turno['cod_turno'] . ' - ' . $turno['nombre_turno'] . '</div>';
                    }
                ?>
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.getElementById("turnosListModal").addEventListener("click", function(e) {
                if (e.target && e.target.matches("div.turno")) {
                    const codTurno = e.target.dataset.codTurno;
                    currentInput.value = codTurno;
                    modal.style.display = "none";
                }
            });
        });
