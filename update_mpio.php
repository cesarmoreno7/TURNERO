...
// Obtener detalles del municipio
if (isset($_GET['cod_municipio'])) {
    $cod_municipio = $_GET['cod_municipio'];
    $municipio_result = mysqli_query($conn, "SELECT * FROM municipio WHERE cod_municipio = $cod_municipio");
    $municipio = mysqli_fetch_assoc($municipio_result);
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "UPDATE municipio SET nom_municipio='$nom_municipio', cod_departamento='$cod_departamento', cod_pais='$cod_pais' WHERE cod_municipio='$cod_municipio'";
...
<div class="container mt-5">
    ...
    <form action="update_mpio.php?cod_municipio=<?= $cod_municipio ?>" method="POST">
        <input type="hidden" name="cod_municipio" value="<?= $municipio['cod_municipio'] ?>">
        <div class="mb-3">
            ...
            <select id="cod_pais" name="cod_pais" class="form-select" required>
                ...
                <?php while ($pais = mysqli_fetch_assoc($paises_result)) { ?>
                    <option value="<?= $pais['cod_pais'] ?>" <?= ($pais['cod_pais'] == $municipio['cod_pais']) ? 'selected' : '' ?>><?= $pais['nom_pais'] ?></option>
                <?php } ?>
            </select>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nom_municipio" class="form-control" value="<?= $municipio['nom_municipio'] ?>" required>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        var cod_departamento = <?= $municipio['cod_departamento'] ?>;
        $.ajax({
            type: 'POST',
            ...
            success: function(html) {
                ...
                $('#cod_departamento').val(cod_departamento);
            }
        });
    });
